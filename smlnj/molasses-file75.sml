(* parse-sml/src/pretty-print/PrettyExpAndDec.sml : 1.1-521.1 *)
(* molasses-file75.sml *)
...
structure PrettyExpAndDec :
  ...
  struct
    ...
    fun showTypbind (front, typbind : Ast.Exp.typbind as {elems, delims}) =
      ...
      in
        rigidVertically (showOne (front, Seq.nth elems 0))
    ...
    and showDec dec =
      let open Ast.Exp in
        case dec of
          DecVal {vall, tyvars, elems, delims} =>
            let
              ...
              val first =
                let val {recc, pat, eq, exp} = Seq.nth elems 0 in
                  separateWithSpaces
                    [ SOME (token vall)
                    , maybeShowSyntaxSeq tyvars token
                    , Option.map token recc
        ...
        | DecDatatype {datatypee, datbind, withtypee} =>
            let val datbinds = showDatbind (datatypee, datbind) in
              case withtypee of
        ...
        | DecMultiple {elems, delims} =>
            let
              fun f i =
                showDec (Seq.nth elems i) ++ (case Seq.nth delims i of
                                                NONE => empty
                                              | SOME semicolon =>
                                                  token semicolon)
            in
        ...
        | DecAbstype {abstypee, datbind, withtypee, withh, dec, endd} =>
            let
              ...
              val bottom =
                group (token withh $$ indent (showDec dec) $$ token endd)
            in
              group
                (case withtypee of
                   SOME {withtypee, typbind} =>
        ...
        | DecReplicateDatatype { left_datatypee
                               , left_id
                               , eq
                               , right_datatypee
                               , right_id
    ...
    and showExp exp =
      let open Ast.Exp in
        ...
        | Infix {left, id, right} =>
            let
              fun showInfix (Infix {left = l', id = i', right = r'}) i r =
                ...
                | showInfix l i r =
                  (case left of
                     Infix _ => showExp l $$ token i ++ space ++ showExp r
                   | _ => showExp l ++ space ++ token i ++ space ++ showExp r)
            in
        ...
        | While {whilee, exp1, doo, exp2} =>
            group
              (group (token whilee $$ indent (showExp exp1) $$ token doo) $$ indent
                                                                               (showExp
                                                                                  exp2))
        ...
        | Handle {exp = expLeft, handlee, elems, delims} =>
            ...
            in
              showExp expLeft \\ (token handlee \\ rigidVertically
                                                     (indent
                                                        (showBranch
                                                           (Seq.nth elems 0)))
                                                     (Seq.map mk
