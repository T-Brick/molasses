(* parse-sml/src/base/Source.sml : 1.1-211.1 *)
(* molasses-file38.sml *)
  ...
  struct
    ...
    fun loadFromFile path =
      let
        ...
        val newlineIdxs =
          ArraySlice.full
            (SeqBasis.filter 2000 (0, n) (fn i => i)
    ...
    fun absoluteStartOffset ({data, ...} : source) =
      let val (_, off, _) = ArraySlice.base data in off end

    fun absoluteEndOffset ({data, ...} : source) =
      let val (_, off, n) = ArraySlice.base data in off + n end
        ...
        let
          ...
          val lineOffset =
            if lineNum = 0 then
              ...
              1 + Seq.nth newlineIdxs (lineNum - 1)
    ...
    fun wholeFile ({data, fileName, newlineIdxs} : source) =
      let val (a, _, _) = ArraySlice.base data in
    ...
    fun wholeLine (s as {data, fileName, newlineIdxs} : source) lineNum1 =
      let
        ...
        val lineEndOffset = Seq.nth newlineIdxs lineNum0
    ...
    fun abut (src1, src2) =
      if
        FilePath.sameFile (fileName src1, fileName src2) andalso absoluteEndOffset
                                                                   src1 = absoluteStartOffset
                                                                            src2
      then
        let
