(* parse-sml/src/pretty-print/PrettyStr.sml : 1.1-166.1 *)
(* molasses-file77.sml *)
  ...
  struct
    ...
    and showStrDec d =
      ...
      | Ast.Str.DecStructure {structuree, elems, delims} =>
          let
            fun showOne (starter, {strid, constraint, eq, strexp}) =
              ...
              \\ separateWithSpaces
                   [ Option.map (fn {sigexp, ...} => showSigExp sigexp)
                       constraint
                   , SOME (token eq)
                   ]
              \\ showStrExp strexp
      ...
      | Ast.Str.DecMultiple {elems, delims} =>
          let
            fun f i =
              showStrDec (Seq.nth elems i) ++ (case Seq.nth delims i of
                                                 NONE => empty
                                               | SOME sc => token sc)
          in
            rigid
      ...
      (** This is MLton-specific. Useful for testing by parsing the entire
        ...
        *)
      | Ast.Str.MLtonOverload { underscore
                              , overload
                              , prec
                              , name
                              , colon
          ...
          let
            ...
            fun showOne (d, e) =
              token d
              ++ space
              ++ token (MaybeLongToken.getToken e)
          in
            group
              (front $$ indent
                          (rigidVertically
