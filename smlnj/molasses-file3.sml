(* parse-sml/src/base/lib/compat/PosixReadFile.sml : 1.1-66.1 *)
(* molasses-file3.sml *)
      ...
      let
        ...
        fun dumpFrom i =
          if i >= length then
            ...
            let val bytesRead = readArr (file, Word8ArraySlice.full buffer) in
              copyToResult i bytesRead; dumpFrom (i + bytesRead)
            end
      in
        dumpFrom 0; close file; ArraySlice.full result
      end

    fun contentsSeq path = contentsSeq' (Char.chr o Word8.toInt) path

    fun contentsBinSeq path = contentsSeq' (fn w => w) path

    fun contents filename =
      let val chars = contentsSeq filename in
        CharVector.tabulate
          (ArraySlice.length chars, fn i => ArraySlice.sub (chars, i))
      end
  end
