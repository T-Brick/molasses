(* parse-sml/src/pretty-print/PrettyUtil.sml : 1.1-68.1 *)
(* molasses-file72.sml *)
  ...
  struct
    ...
    fun sequence openn delims close (xs : doc Seq.t) =
      if Seq.length xs = 0 then
        ...
        in
          group
            (Seq.iterate op // top (Seq.map f (Seq.zip (delims, Seq.drop xs 1))) // token
                                                                                      close)
        end
     

    fun separateWithSpaces (items : doc option list) : doc =
      let val items : doc list = List.mapPartial (fn x => x) items in
        case items of
          [] => empty
        | first :: rest =>
