(* parse-sml/src/base/lib/SampleSort.sml : 1.1-201.1 *)
(* molasses-file18.sml *)
...
structure SampleSort :>
  ...
  struct
    ...
    fun matrixDandC baseCase (threshold, num_rows, num_cols) =
      let
        fun r (rs, rl, cs, cl) =
          if (rl * cl < threshold) then
            baseCase (rs, rl, cs, cl)
          else if (cl > rl) then
            ( par
                ( fn () => r (rs, rl, cs, cl div 2)
            ...
            ( par
                ( fn () => r (rs, rl div 2, cs, cl)
                , fn () => r (rs + (rl div 2), rl - (rl div 2), cs, cl)
                )
            ; ()
            )
      in
        r (0, num_rows, 0, num_cols)
      end
                 ...
                 (fn j =>
                    update
                      ...
                      , j
                        * num_rows
                        + i
                      , sub (SS, (i * num_cols + j + offset))
                      )))
      in
        (matrixDandC baseCase (seq_threshold, num_rows, num_cols); AS.full (R))
      end

    (* transposes a matrix of blocks given source and destination pairs *)
    fun transposeBlocks ( S
                        , source_offsets
                        , dest_offsets
                        , counts
                        , num_rows
      ...
      let
        ...
        fun baseCase (row_start, row_len, col_start, col_len) =
          for (row_start, row_start + row_len)
            (fn i =>
               for (col_start, col_start + col_len)
                 (fn j =>
                    let
                      val pa =
                        offset + AS.sub (source_offsets, i * num_cols + j)
                      val pb = Seq.nth dest_offsets (j * num_rows + i)
                      val l = Seq.nth counts (i * num_cols + j)
                    in
                      for (0, l) (fn k => update (R, pb + k, sub (SS, pa + k)))
                    end))
      in
        (matrixDandC baseCase (seq_threshold, num_rows, num_cols); AS.full (R))
      end
    ...
    fun mergeWithSamples cmp (A, S, C) =
      let
        ...
        val n = AS.length A
        fun merge (i, j) =
          if (j = num_samples) then
            AS.update (C, j, n - i)
          else
            let
              fun merge' (i) =
                if
                  (i < n andalso cmp (AS.sub (A, i), AS.sub (S, j)) = LESS)
                then
                  merge' (i + 1)
                else
                  i
    ...
    fun sort cmp A =
      let
        ...
        val sample =
          Seq.tabulate (fn i => AS.sub (A, i * sample_stride)) sample_size
        ...
        (* take a subsample *)
        val sub_sample =
          Seq.tabulate (fn i => AS.sub (sample, (i + 1) * over_sample))
            (num_buckets - 1)
        ...
        fun bucket_offset i =
          if (i = num_buckets) then
            ...
            AS.sub (dest_offsets, i * num_blocks)

        (* sort the buckets *)
        val _ =
          parallelFor 1 (0, num_buckets)
            (fn i =>
               let
