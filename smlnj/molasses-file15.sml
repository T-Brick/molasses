(* parse-sml/src/base/lib/Quicksort.sml : 1.1-139.1 *)
(* molasses-file15.sml *)
  ...
  struct
    ...
    fun sortRange grainsize (array, start, n, compare) =
      let
        ...
        fun swap (i, j) =
          let val tmp = item i in set (i, item j); set (j, tmp) end

        (* same as swap(j,k); swap(i,j) *)
        fun rotate (i, j, k) =
          let val tmp = item k in
        ...
        fun twoPivots (a, n) =
          let
            fun sortToFront (size) =
              let
                val m = n div (size + 1)
                fun toFront (i) =
                  if (i < size) then
                    (swap (a + i, a + m * (i + 1)); toFront (i + 1))
                  else
                    ()
              in
          ...
          in
            if (n < 80) then
              (if cmp (a, a + n - 1) = GREATER then
                 swap (a, a + n - 1)
               else
                 ())
            else
              (sortToFront (5); swap (a + 1, a); swap (a + 3, a + n - 1))
          end
        ...
        fun split3 (a, n) =
          let
            ...
            fun loop (l, m, r) =
              if (m > r) then
                (l, m)
              else if cmp (m, p1) = LESS then
                (swap (m, l); loop (l + 1, m + 1, r))
              else
                (if cmp (m, p2) = GREATER then
                   (if cmp (r, p1) = LESS then
                      (rotate (l, m, r); loop (l + 1, m + 1, right (r - 1)))
                    else
                      (swap (m, r); loop (l, m + 1, right (r - 1))))
                 else
                   loop (l, m + 1, r))
        ...
        fun qsort (a, n) =
          if (n < 16) then
            ...
            let val (l, m, doMid) = split3 (a, n) in
              if (n <= grainsize) then
                ( qsort (a, l - a)
                ; (if doMid then
                     qsort (l, m - l)
    ...
    fun sortInPlaceG grainsize compare aslice =
      let val (a, i, n) = AS.base aslice in
        sortRange grainsize (a, i, n, compare)
      end

    fun sortG grainsize compare aslice =
      let val result = AS.full (ForkJoin.alloc (AS.length aslice)) in
