(* parse-sml/src/base/lib/CountingSort.sml : 1.1-135.1 *)
(* molasses-file19.sml *)
...
structure CountingSort :>
  sig
    ...
    val sort : 'a seq
                 -> (int -> int)
               (* bucket id of ith element *)
                  -> int
               (* number of buckets *)
                  -> 'a seq * int seq
  (* sorted, bucket offsets *)
  end =
  struct
    ...
    fun seqSortInternal In Out Keys Counts genOffsets =
      ...
      in
        ...
        loop (0, m) 0
          (fn (s, i) =>
             let val t = sub (Counts, i) in update (Counts, i, s); s + t end);
        ...
        for (0, n)
          (fn i =>
             let
               ...
               val k = sub (Counts, j)
             in
               update (Counts, j, k + 1); update (Out, k, sub (In, i))
             end);

        if genOffsets then
          ( forBackwards (0, m - 1)
              (fn i => update (Counts, i + 1, sub (Counts, i)))
          ; update (Counts, 0, 0)
          ; 0
          )
        else
          loop (0, m) 0
            (fn (s, i) =>
               let val t = sub (Counts, i) in (update (Counts, i, t - s); t) end)
      end
      ...
      in
        if (numBlocks <= 1 orelse n < SeqThreshold) then
          ...
          let
            ...
            val _ =
              ForkJoin.parfor 1 (0, numBlocks)
                (fn i =>
                   let
                     val start = Int.min (i * blockSize, n)
                     val len = Int.min ((i + 1) * blockSize, n) - start
                   in
                     seqSortInternal (AS.subslice (In, start, SOME (len)))
                       (AS.subslice (B, start, SOME (len)))
                       (fn i => Keys (i + start))
                       (AS.subslice
                          (Counts, i * numBucketsPad, SOME (numBucketsPad)))
                       false;
                     ()
                   end)
            val (sourceOffsets, _) = Seq.scan op + 0 Counts
            val transCounts =
              SampleSort.transpose (Counts, numBlocks, numBucketsPad)
            val (destOffsets, _) = Seq.scan op + 0 transCounts
            val C =
              SampleSort.transposeBlocks
                ( B
                , sourceOffsets
                , destOffsets
                , Counts
              ...
              Seq.tabulate
                (fn i =>
                   if (i = numBuckets) then
                     ...
                     AS.sub (destOffsets, i * numBlocks))
                (numBuckets + 1)
          in
            (C, bucketOffsets)
          end
