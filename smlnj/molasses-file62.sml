(* parse-sml/src/parse/Parser.sml : 1.1-634.1 *)
(* molasses-file62.sml *)
  ...
  struct
    ...
    fun parseWithInfdict infdict src =
      let
        ...
        fun consume_sigExp infdict i = ParseSigExpAndSpec.sigexp toks infdict i
        fun consume_sigSpec infdict i = ParseSigExpAndSpec.spec toks infdict i
        ...
        fun consume_strexpConstraint infdict strexp colon i =
          let val (i, sigexp) = consume_sigExp infdict i in
            ( i
            , Ast.Str.Constraint
                {strexp = strexp, colon = colon, sigexp = sigexp}
            )
          end
        ...
        and consume_strexpStruct infdict structt i : (int * Ast.Str.strexp) =
          ...
          in
            ( i
            , Ast.Str.Struct {structt = structt, strdec = strdec, endd = endd}
            )
          end
        ...
        and consume_strexpFunApp infdict funid lparen i =
          if
            check Token.isDecStartToken i orelse check Token.isStrDecStartToken
                                                   i orelse isReserved
                                                              Token.CloseParen
                                                              i
          then
          ...
          let
            val (i, strexp) =
              if isReserved Token.Struct i then
                ...
                check Token.isStrIdentifier i andalso isReserved Token.OpenParen
                                                        (i + 1)
              then
        ...
        and consume_afterStrexp infdict strexp i =
          let
            val (again, (i, strexp)) =
              if
                isReserved Token.Colon i orelse isReserved Token.ColonArrow i
              then
        ...
        and consume_exactlyOneStrdec (i, infdict) : ((int * InfixDict.t) * Ast.Str.strdec) =
        ...
        and consume_strdec (i, infdict) =
          let
            ...
            fun continue (i, _) =
              check Token.isDecStartToken i orelse check
                                                     Token.isStrDecStartToken
                                                     i
            ...
            val result =
              case Seq.length strdecs of
                0 => Ast.Str.DecEmpty
              | 1 =>
                  let val (dec, semicolon) = Seq.nth strdecs 0 in
          ...
          let
            ...
            fun parse_funarg infdict i =
              if not (check Token.isStrIdentifier i) then
                let val (i, spec) = consume_sigSpec infdict i in
        ...
        fun consume_topDecOne (i, infdict) : ((int * InfixDict.t) * Ast.topdec) =
          if isReserved Token.Signature at i then
            ...
            check Token.isStrDecStartToken at i orelse check
                                                         Token.isDecStartToken at i
          then
            let val (xx, strdec) = consume_strdec (i, infdict) in
              (xx, Ast.StrDec strdec)
            end
          else if isReserved Token.Underscore i then
            let val (i, strdec) = consume_MLtonOverload (tok i) (i + 1) in
        ...
        fun parse_topDecMaybeSemicolon (i, infdict) =
          let
            ...
            val (i, semicolon) =
              ParseSimple.maybeReserved toks Token.Semicolon i
          in
    ...
    fun parse src =
      let val (_, ast) = parseWithInfdict InfixDict.initialTopLevel src in
        ast
      end
