(* parse-sml/src/base/lib/Tokenize.sml : 1.1-59.1 *)
(* molasses-file22.sml *)
  ...
  struct
      ...
      let
        ...
        fun check i =
          if (i = n) then
            ...
            let
              val i1 = f (Seq.nth s i)
              val i2 = f (Seq.nth s (i - 1))
            in
              (i1 andalso not i2) orelse (i2 andalso not i1)
            end
        val ids =
          ArraySlice.full (SeqBasis.filter 10000 (0, n + 1) (fn i => i) check)
    ...
    fun tokensSeq f s =
      let
        ...
        fun token i = let val (lo, hi) = g i in Seq.subseq s (lo, hi - lo) end
      in
