(* parse-sml/src/parse/ParseSigExpAndSpec.sml : 1.1-750.1 *)
(* molasses-file61.sml *)
        ...
        fun consume_sigExpWhereType sigexp i =
          let
            fun nextIsWhereOrAndType i =
              (isReserved Token.Where i orelse isReserved Token.And i) andalso (isReserved
                                                                                  Token.Type
                                                                                  (i + 1))
          ...
          let
            val (i, sigexp) =
              if isReserved Token.Sig i then
                ...
                let val (i, sigid) = parse_sigid i in
    ...
    and parse_spec toks infdict i =
      let
            ...
            if isReserved Token.Equal i then
              ...
              in
                consume_sigSpecTypeAbbreviation typee
                  {tyvars = tyvars, tycon = tycon, eq = eq, ty = ty}
                  i
              end
            else
        ...
        fun consume_sigSpecDatatypeDeclarationOrReplication i =
          if
            (isReserved Token.OpenParen i (* datatype ('a, ...) foo *) orelse check
                                                                                Token.isTyVar
                                                                                i (* datatype 'a foo *) orelse ((* datatype foo = A *) check
                                                                                                                                        Token.isValueIdentifierNoEqual
                                                                                                                                        i andalso isReserved
                                                                                                                                                    Token.Equal
                                                                                                                                                    (i + 1) andalso not
                                                                                                                                                                      (isReserved
                                                                                                                                                                         Token.Datatype
          ...
          let
            ...
            val (i, specs) =
              parse_zeroOrMoreWhile (fn i => check Token.isSigSpecStartToken i)
                (parse_two (consume_oneSigSpec, consume_maybeSemicolon))
                i
            ...
            val result =
              case Seq.length specs of
                0 => Ast.Sig.EmptySpec
              | 1 =>
                  let val (spec, semicolon) = Seq.nth specs 0 in
                    if isSome semicolon then makeSpecMultiple () else spec
                  end
              | _ => makeSpecMultiple ()
          in
            (i, result)
