...
structure Molasses :
  sig
    ...
    type result = {cm : GenFile.t option, top : GenFile.t list}
  ...
  struct
    ...
    local
      ...
      fun writeOut (dir, rel) out =
        let
          ...
          fun write () =
            let val outstream = TextIO.openOut file in
              TextIO.output (outstream, GenFile.toString out);
              TextIO.output (outstream, "\n");
              TextIO.closeOut outstream
            end
        in
          if exists (OS.Path.dir file) (OS.Path.file file) then
            Control.print ("Tried writing " ^ file ^ " which exists!\n")
          else
            write ()
        end
      ...
      fun maker pathmap file outdir =
        let
          ...
          fun prepDirectory () =
            (OS.Process.system ("rm -rf " ^ outdir); OS.FileSys.mkDir outdir)
        ...
        in
          case cm of
            NONE => (* no cm implies all top-level *) {cm = NONE, top = all}
          | SOME cm_f =>
              ( write (GenFile.TopLevel toplevel, total + 1)
              ; {cm = SOME (GenFile.CM cm_f), top = [GenFile.TopLevel toplevel]}
              )
        end
    ...
    in
      fun makeTo pathmap outdir file =
        (maker pathmap o checkFile) outdir file
          handle
              UnknownFile file =>
                if # get Control.recover_src () then
