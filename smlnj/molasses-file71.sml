(* parse-sml/src/pretty-print/TokenDoc.sml : 1.1-537.1 *)
(* molasses-file71.sml *)
      ...
      let
        fun blankLinesAbove d n =
          if n <= 0 then
            ...
            blankLinesAbove (Above (false, space, d)) (n - 1)
        ...
        fun doDoc doc =
          ...
          | Indent d =>
              let val (first, d', last) = doDoc d in
                (first, Indent d', last)
              end

          | Group d =>
              let val (first, d', last) = doDoc d in (first, Group d', last) end
          ...
          | Rigid d =>
              let val (first, d', last) = doDoc d in (first, Rigid d', last) end
    ...
    fun raiseComments doc =
      let
        ...
        fun loop doc =
          ...
          | Indent doc =>
              let val (hasToks, doc', cb, ca) = loop doc in
                if hasToks then
    ...
    fun insertComments doc =
      let
        ...
        fun insertOnlyAfterLast mode d =
          ...
          | Indent d =>
              let val (foundIt, d') = insertOnlyAfterLast mode d in
                (foundIt, Indent d')
              end
          | Token tok => (true, insertCommentsAfterTok mode tok)
          | Beside (d1, d2) =>
              let val (foundIt, d2') = insertOnlyAfterLast BesideMode d2 in
                if foundIt then
                  (true, Beside (d1, d2'))
                else
                  let val (foundIt, d1') = insertOnlyAfterLast mode d1 in
          ...
          | Above (b, d1, d2) =>
              let val (foundIt, d2') = insertOnlyAfterLast AboveMode d2 in
                if foundIt then
                  (true, Above (b, d1, d2'))
                else
                  let val (foundIt, d1') = insertOnlyAfterLast mode d1 in
    ...
    fun tokenToStringDoc tabWidth tok =
      ...
      in
        if Seq.length pieces = 1 then
          ...
          (false, Seq.iterate StringDoc.aboveOrSpace StringDoc.empty pieces)
      end
    ...
    fun toStringDoc {tabWidth} d =
      let
        ...
        fun loop d =
          ...
          | Group d =>
              let val (groupable, d') = loop d in
                if groupable then (true, StringDoc.group d') else (false, d')
              end

          | Rigid d =>
              let val (_, d') = loop d in (false, StringDoc.rigid d') end
