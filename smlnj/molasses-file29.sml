(* parse-sml/src/base/lib/NewWaveIO.sml : 1.1-268.1 *)
(* molasses-file29.sml *)
...
structure NewWaveIO :
  ...
  struct
    ...
    fun err msg = raise Fail ("NewWaveIO: " ^ msg)
    ...
    fun compress boost (snd as {sr, data} : sound) =
      if boost < 1.0 then
        ...
        let
          ...
          fun transfer x =
            let (* normalized *) val x' = Real.abs (x / maxA') in
              (* compressed *)
      ...
      let
          ...
          (* found it! *)
            ...
            let
              ...
              val chunkName =
                CharVector.tabulate
                  ( 4
                  , fn i => Char.chr (Word8.toInt (Seq.nth bytes (offset + i)))
                  )
            in
              if chunkSize < 0 then
        ...
        val _ =
          if Seq.length bytes >= 12 then
            ...
            err "not enough bytes for RIFF header"

        val offset = 0
        val riff = 0wx52494646
        (* ascii "RIFF", big endian *)
        val _ =
          if Parse.r32b bytes offset = riff then
            ()
          else
            err "expected 'RIFF' chunk ID"
        ...
        val _ =
          if Seq.length bytes >= totalFileSize then
            ...
            err
              ("expected "
               ^ Int.toString totalFileSize
               ^ " bytes but the file is only "
               ^ Int.toString (Seq.length bytes))

        val offset = 8
        val wave = 0wx57415645
        (* ascii "WAVE" big endian *)
        val _ =
          if Parse.r32b bytes offset = wave then
            ()
          else
            err "expected 'WAVE' format"
        ...
        val fmtId = 0wx666d7420
        (* ascii "fmt " big endian *)
        ...
        val _ =
          if fmtChunkSize >= 16 then
            ...
            err "expected 'fmt ' chunk to be at least 16 bytes"
        ...
        val _ =
          if audioFormat = 1 then
            ...
            err
              ("expected PCM audio format, but found 0x" ^ Int.fmt StringCvt.HEX
                                                             audioFormat)
        ...
        val dataId = 0wx64617461
        (* ascii "data" big endian *)
        ...
        val _ =
          if dataChunkStart + 8 + dataSize <= totalFileSize then
            ...
            err ("badly formatted data chunk: unexpected end-of-file")
        ...
        fun readChannel i j =
          readSample
            (dataStart
             + j * (numChannels * bytesPerSample)
             + i * bytesPerSample)

        val rawData =
          AS.full
            (SeqBasis.tabulate 1000 (0, numSamples)
               (fn j =>
                  Util.loop (0, numChannels) 0.0
                    (fn (s, i) => s + readChannel i j)))
    ...
    fun writeSound ({sr, data} : sound) path =
      ...
      in
        ...
        w32b fmtId;
        w32l 0w16;
        (* 16 remaining bytes in subchunk *)
        w16l 0w1;
        (* audio format PCM = 1 *)
        w16l 0w1;
        (* 1 channel (mono) *)
        ...
        w32b dataId;
        w32l (Word32.fromInt (2 * Seq.length data));
        (* number of data bytes *)

        Util.for (0, Seq.length data)
          (fn i =>
             let
