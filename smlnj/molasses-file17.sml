(* parse-sml/src/base/lib/TreeMatrix.sml : 1.1-154.1 *)
(* molasses-file17.sml *)
...
structure TreeMatrix :
  ...
  struct
      ...
      let
        fun tab n (row, col) =
          if n <= 64 then
            Leaf
              ( n
              , Array.tabulate (n * n, fn i => f (row + i div n, col + i mod n))
              )
          else
            let
              val half = n div 2
    ...
    fun writeFlatten (result, start, rowskip) m =
      case m of
        Leaf (n, s) =>
          let fun idx i = start + (i div n) * rowskip + (i mod n) in
      ...
      | Node (n, m11, m12, m21, m22) =>
          ( par4
              ...
              , fn _ =>
                  writeFlatten (result, start + (n div 2) * rowskip, rowskip)
                    m21
              , fn _ =>
                  writeFlatten
                    (result, start + (n div 2) * (rowskip + 1), rowskip)
                    m22
              )
          ; ()
          )
    ...
    fun flatten m =
      ...
      in
        writeFlatten (result, 0, n) m; result
      end
    ...
    fun flatmultiply n (s, t, output) =
      let
        ...
        fun loopi i hi = if i >= hi then () else (update i; loopi (i + 1) hi)
      in
        loopi 0 (n * n)
      end
    ...
    fun multiply (a, b) =
      let val c = tabulate (sidelength a) (fn _ => 0.0) in
        multiply' (a, b, c); c
      end
