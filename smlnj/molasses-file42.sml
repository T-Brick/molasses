(* parse-sml/src/base/Error.sml : 1.1-178.1 *)
(* molasses-file42.sml *)
...
structure Error :
  sig
    ...
    datatype element =
    Paragraph of string | ItemList of string list | SourceReference of Source.t
    ...
    fun showElement highlighter desiredWidth e =
      ...
      in
        ...
        | ItemList lns =>
            let
              fun showLine ln =
                "  " ^ TextFormat.textWrap (desiredWidth - 4)
                         (bullet
                          ^ " "
                          ^ ln)
            in
            ...
            let
              ...
              val _ =
                if lineNum = lineNum' orelse colEnd = 1 then
                  ...
                  raise
                    Fail "ErrorReport.show: end of position past end of line"
              ...
              val numTabsBefore =
                List.length
                  (List.filter (fn c => c = #"\t")
                     (String.explode
                        (Source.toString (Source.take line colOffset))))
              ...
              val filestyle = TCS.foreground lightblue
            (*o TCS.background lightgray*)
            in
              TCS.concatWith ($ "\n")
