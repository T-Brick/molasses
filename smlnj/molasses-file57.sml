(* parse-sml/src/parse/ParseTy.sml : 1.1-262.1 *)
(* molasses-file57.sml *)
    ...
    fun ty toks start =
      let
                ...
                let
                  ...
                  val (i, ty) =
                    parse_tyWithRestriction
                      {permitArrows = true, permitTuple = true}
                      (i + 1)
                in
        ...
        (** ty
          ...
          *   ty * ...        -- tuple
          *)
        and parse_afterTy (restriction as { permitArrows : bool
                                          , permitTuple : bool
                                          }) ty i =
          let
            val (again, (i, ty)) =
        ...
        and parse_tyTuple tys delims i =
          let
            ...
            val (i, ty) =
              parse_tyWithRestriction
                {permitArrows = false, permitTuple = false}
                i
            val tys = ty :: tys
        ...
        and parse_tyEndParensOrSequence leftParen tys delims i =
          let val rightParen = tok (i - 1) in
            case (tys, delims) of
              ([ty], []) =>
                ( i
                , Ast.Ty.Parens {left = leftParen, ty = ty, right = rightParen}
                )
