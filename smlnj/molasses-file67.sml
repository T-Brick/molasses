(* parse-sml/src/parse-mlb/MLBParser.sml : 1.1-623.1 *)
(* molasses-file67.sml *)
...
structure MLBParser :
  ...
  struct
    ...
    fun checkSML toks f i =
      i < Seq.length toks andalso case MLBToken.getClass (Seq.nth toks i) of
                                    MLBToken.SML c =>
                                      f
                                        (Token.fromPre
                                           (Token.Pretoken.make
      ...
      let
        ...
        fun parse_decPathFromString i =
          let
            ...
            val _ =
              if
                n >= 2 andalso Source.nth thisSrc 0 = #"\"" andalso Source.nth
                                                                      thisSrc
                                                                      (n - 1) = #"\""
              then
                ()
          ...
          let
            ...
            val (i, basdecs) =
              ParserCombinators.zeroOrMoreWhile continue
                (ParserCombinators.two
                   (parse_exactlyOneDec, parse_maybeSemicolon))
                i
            ...
            val result =
              case Seq.length basdecs of
                0 => MLBAst.DecEmpty
              | 1 =>
                  let val (dec, semicolon) = Seq.nth basdecs 0 in
