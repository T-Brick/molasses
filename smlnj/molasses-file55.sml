(* parse-sml/src/parse/FixExpPrecedence.sml : 1.1-186.1 *)
(* molasses-file55.sml *)
...
structure FixExpPrecedence :
  sig val maybeRotateLeft : Ast.Exp.exp -> Ast.Exp.exp end =
  struct
    ...
    type replacer =
      {exp : Ast.Exp.exp, replaceWith : Ast.Exp.exp -> Ast.Exp.exp}
    ...
    fun getInfo e =
      let open Ast.Exp in
        ...
        | Raise {raisee, exp} =>
            let fun replaceRightWith e = Raise {raisee = raisee, exp = e} in
              F
                { prec = 6
                , left = NONE
    ...
    fun maybeRotateLeft e =
      let val F {prec = rootPrecedence, right, ...} = getInfo e in
        case right of
          NONE => e
        | SOME {exp = rootRight, replaceWith = rootReplaceRightWith} =>
            let val F {prec = rightPrecedence, left, ...} = getInfo rootRight in
              case left of
                NONE => e
