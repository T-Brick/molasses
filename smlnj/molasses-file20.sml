(* parse-sml/src/base/lib/RadixSort.sml : 1.1-151.1 *)
(* molasses-file20.sml *)
...
structure RadixSort :>
  ...
  struct
      ...
      let
        fun loop s i =
          if i < 0 then
            ...
            loop (# 1 (CountingSort.sort s (bucket s i) numBuckets)) (i - 1)
      in
        loop s (numPasses - 1)
      end
      ...
      let
        ...
        fun msdSort' s pass lo hi =
          if pass = numPasses then
            ForkJoin.parfor AS.GRAIN (0, hi - lo)
              (fn i => AS.ASupdate (result, lo + i, AS.nth s i))
          else
            let
              val (s', offsets) = CountingSort.sort s (bucket s pass) numBuckets
            in
              ForkJoin.parfor AS.GRAIN (0, numBuckets)
                (fn i =>
                   let
    ...
    fun par3 (a, b, c) =
      let val ((ar, br), cr) = ForkJoin.par (fn _ => ForkJoin.par (a, b), c) in
        (ar, br, cr)
      end
