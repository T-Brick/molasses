(* parse-sml/src/lex-mlb/MLBLexer.sml : 1.1-186.1 *)
(* molasses-file64.sml *)
    ...
    fun next (src : Source.t) : MLBToken.Pretoken.t option =
      let
        ...
        fun isString str i =
          Source.length src
          - i
          >= String.size str andalso Util.all (0, String.size str)
                                       (fn j =>
        ...
        fun loop_toplevel i =
          ...
          else if isString "bas" i then
            loop_after_bas (i + 3)

          else if
            isString "ann" i andalso not
                                       (check LexUtils.isValidUnquotedPathChar
                                          (i + 3))
          then
            success (mkr MLBToken.Ann (i, i + 3))

          else if
            isString "_prim" i andalso not
                                         (check LexUtils.isValidUnquotedPathChar
                                            (i + 5))
          then
            success (mkr MLBToken.UnderscorePrim (i, i + 5))

          else if check LexUtils.isValidUnquotedPathChar i then
            loop_maybePath {start = i} (i + 1)

          else if check Char.isSpace i then
            loop_toplevel (i + 1)
        ...
        and loop_after_bas i =
          if
            isString "is" i andalso not
                                      (check LexUtils.isValidUnquotedPathChar
                                         (i + 2))
          then
            success (mkr MLBToken.Basis (i - 3, i + 2))

          else if not (check LexUtils.isValidUnquotedPathChar i) then
