(* parse-sml/src/base/TextFormat.sml : 1.1-90.1 *)
(* molasses-file33.sml *)
  ...
  struct
    ...
    fun leftPadWith char desiredWidth str =
      if String.size str >= desiredWidth then
        ...
        repeatChar (desiredWidth - String.size str) char ^ str

    fun rightPadWith char desiredWidth str =
      if String.size str >= desiredWidth then
        str
      else
        str ^ repeatChar (desiredWidth - String.size str) char
    ...
    fun spreadWith char desiredWidth {left, right} =
      if String.size left + String.size right >= desiredWidth then
        ...
        let val count = desiredWidth - (String.size left + String.size right) in
          left ^ repeatChar count char ^ right
        end
