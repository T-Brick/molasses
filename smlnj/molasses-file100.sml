(* src/gen/internal/CMGenerator.sml : 1.1-173.1 *)
(* molasses-file100.sml *)
structure CMGenerator :> INTERNAL_GENERATOR =
  struct
    ...
    type acc =
      ...
      , smls : WFile.t list
      , future : WFile.future
      , filter : StrExport.t list * bool
      (* export list, does rename *)
      , gened : Generated.t
      }
    val emptyAcc =
      { cms = []
    ...
    fun makeList (fs as (dir, pathmap)) cfile tok_opt create acc file elems =
      let
        fun folder ((d, src_opt), a) =
          ((case src_opt of
              NONE => (fn x => x)
            | SOME src => (fn x => createMark (src, x))) o create fs NONE a)
            d
    ...
    fun makeLocal (dir, pathmap) cfile create acc (locall, inn, endd) ( basdec1
      ...
      let
        ...
        val (old_cm, new_acc) =
          if isAccEmpty acc then
            (new_cm, acc)
          else
            let val old = FileName.newCM () in
