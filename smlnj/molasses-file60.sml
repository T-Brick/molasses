(* parse-sml/src/parse/ParseExpAndDec.sml : 1.1-1458.1 *)
(* molasses-file60.sml *)
...
structure ParseExpAndDec :
  ...
  struct
      ...
      in
        case right of
          Ast.Exp.Infix {left = rLeft, id = rId, right = rRight} =>
            if
              hp (rId, id) orelse (sp (rId, id) andalso bothRight (rId, id))
            then
              default
            else if
              hp (id, rId) orelse (sp (rId, id) andalso bothLeft (rId, id))
            then
              Ast.Exp.Infix
    ...
    fun endsCurrentExp infdict restrict tok =
      Token.endsCurrentExp tok orelse (not (Restriction.infOkay restrict) andalso Token.isValueIdentifier
                                                                                    tok andalso InfixDict.isInfix
                                                                                                  infdict
                                                                                                  tok) orelse (not
                                                                                                                 (Restriction.anyOkay
                                                                                                                    restrict) andalso case Token.getClass
                                                                                                                                             tok of
                                                                                                                                        Token.Reserved rc =>
                                                                                                                                          List.exists
                                                                                                                                            (fn rc' =>
    ...
    fun dec {forceExactlyOne} toks (start, infdict) =
      let
        ...
        fun parse_pat infdict restriction i = PP.pat toks infdict restriction i
          ...
          let
            ...
            val result =
              case Seq.length decs of
                0 => Ast.Exp.DecEmpty
              | 1 =>
                  let val (dec, semicolon) = Seq.nth decs 0 in
                    if isSome semicolon then makeDecMultiple () else dec
                  end
              | _ => makeDecMultiple ()
          in
            ((i, infdict), result)
        ...
        and consume_decDatatypeDeclarationOrReplication (i, infdict) =
          if
            (isReserved Token.OpenParen at i (* datatype ('a, ...) foo *) orelse check
                                                                                   Token.isTyVar at i (* datatype 'a foo *) orelse ((* datatype foo = A *) check
                                                                                                                                                            Token.isValueIdentifierNoEqual at i andalso isReserved
                                                                                                                                                                                                          Token.Equal at (i + 1) andalso not
                                                                                                                                                                                                                                           (isReserved
                                                                                                                                                                                                                                              Token.Datatype at (i + 2))))
          then
            let
            ...
            val (i, exp) =
              if check Token.isConstant at i then
                ...
                ParserUtils.tokError toks
                  ...
                  , what = "Unexpected token."
                  , explain =
                      SOME "Reserved keyword used. Try changing variable names."
                  }
              else
                ParserUtils.tokError toks
                  { pos = i
        ...
        and consume_afterExp infdict restriction exp i =
          let
            val (again, (i, exp)) =
              if
                i >= numToks orelse check (endsCurrentExp infdict restriction) at i
              then
                (false, (i, exp))

              else if isReserved Token.Colon at i then
                (true, consume_expTyped exp (i + 1))

              else if isReserved Token.Handle at i then
              ...
              then
                ...
                Restriction.infOkay restriction andalso Ast.Exp.isInfExp exp andalso check
                                                                                       Token.isValueIdentifier at i andalso InfixDict.isInfix
                                                                                                                              infdict
                                                                                                                              (tok
                                                                                                                                 i)
              then
              ...
              else if Restriction.appOkay restriction then
          ...
          in
            if i >= numToks then
              ...
              let
                val nextTok = tok i
                val isMLtonThing =
                  Token.isIdentifier nextTok andalso case Source.toString
                                                            (Token.getSource
                                                               nextTok) of
                                                       "prim" => true
                                                     | "import" => true
        ...
        and consume_expSelect hash i =
          let val (i, lab) = parse_recordLabel i in
            (i, Ast.Exp.Select {hash = hash, label = lab})
          end
            ...
            in
              if isReserved Token.CloseParen at i then
                ...
                let
                  ...
                  val (i, {elems, delims}) =
                    parse_oneOrMoreDelimitedByReserved
                      {parseElem = parseElem, delim = delimType}
                      i
