(* parse-sml/src/base/lib/SeqBasis.sml : 1.1-205.1 *)
(* molasses-file8.sml *)
...
structure SeqBasis :
  sig
    ...
    val scan : grain
                 ...
                 -> (int -> 'a)
                 -> 'a array
    (* length N+1, for both inclusive and exclusive scan *)
    ...
    val tabFilter : grain -> (int * int) -> (int -> 'a option) -> 'a array
  end =
  struct
    ...
    *)
    ...
    fun foldl g b (lo, hi) f =
      if lo >= hi then
        ...
        let val b' = g (b, f lo) in foldl g b' (lo + 1, hi) f end
      ...
      if hi - lo <= grain then
        ...
        let
          ...
          fun red i j =
            ...
            | n =>
                let val mid = i + (j - i) div 2 in
                  g (par (fn _ => red i mid, fn _ => red mid j))
                end
        in
          red 0 m
        end
      ...
      if hi - lo <= grain then
        ...
        let
          ...
          val sums =
            tabulate 1 (0, m)
              (fn i =>
                 let val start = lo + i * k in
                   foldl g b (start, Int.min (start + k, hi)) f
                 end)
      ...
      let
        ...
        val counts =
          tabulate 1 (0, m)
            (fn i =>
               let val start = lo + i * k in
                 count (start, Int.min (start + k, hi)) 0
               end)
        ...
        fun filterSeq (i, j) c =
          if i >= j then
            ...
            (upd result c (f i); filterSeq (i + 1, j) (c + 1))
          else
            filterSeq (i + 1, j) c
      in
        parfor 1 (0, m)
          (fn i =>
             let val start = lo + i * k in
               filterSeq (start, Int.min (start + k, hi)) (nth offsets i)
             end);
        result
      end
    ...
    fun tabFilter grain (lo, hi) (f : int -> 'a option) =
        ...
        val counts =
          tabulate 1 (0, m)
            (fn i =>
               let
                 val last =
                   filterSeq (lo + i * k, lo + Int.min ((i + 1) * k, n), i * k)
               in
                 last - i * k
               end)
      ...
      in
        ...
        parfor (n div (Int.max (outSize, 1))) (0, m)
          (fn i =>
             let
               val soff = i * k
             ...
             in
               Util.for (0, size)
                 (fn j => A.update (result, doff + j, A.sub (tmp, soff + j)))
             end);
        result
      end
