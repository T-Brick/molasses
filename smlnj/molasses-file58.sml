(* parse-sml/src/parse/ParsePat.sml : 1.1-443.1 *)
(* molasses-file58.sml *)
    ...
    fun pat toks infdict restriction start =
      let
            ...
            val (again, (i, pat)) =
              if
                (** Annoying edge case with '='... we can use it in an infix
                  ...
                  *)
                Restriction.infOkay restriction andalso check
                                                          Token.isValueIdentifierNoEqual
                                                          i andalso InfixDict.isInfix
                                                                      infdict
                                                                      (tok i)
              then
                ...
                Restriction.appOkay restriction andalso Ast.Pat.okayForConPat
                                                          pat andalso check
                                                                        Token.isAtPatStartToken
                                                                        i
              then
              ...
              then
                ...
                isReserved Token.As i andalso Restriction.anyOkay restriction andalso Ast.Pat.okayForAsPat
                                                                                        pat
              then
                ( true
          ...
          if isReserved Token.DotDotDot i then
            if isReserved Token.CloseCurlyBracket (i + 1) then
              ...
              ParserUtils.tokError toks
                ...
                , what = "Unexpected token."
                , explain =
                    SOME "This can only appear at the end of the record."
                }
          else if
        ...
        and consume_patAs infdict {opp, id, ty} ass i =
          let val (i, pat) = consume_pat infdict Restriction.None i in
            ( i
            , Ast.Pat.Layered
                {opp = opp, id = id, ty = ty, ass = ass, pat = pat}
            )
          end
        ...
        and consume_patCon infdict {opp, id} i =
          let val (i, atpat) = consume_pat infdict Restriction.At i in
            (i, Ast.Pat.Con {opp = opp, id = id, atpat = atpat})
          end
        ...
        and consume_patTyped infdict pat colon i =
          let val (i, ty) = parse_ty i in
            (i, Ast.Pat.Typed {pat = pat, colon = colon, ty = ty})
          end
        ...
        and consume_patInfix infdict leftPat vid i =
          let val (i, rightPat) = consume_pat infdict Restriction.Inf i in
            (i, makeInfixPat infdict (leftPat, vid, rightPat))
          end
