(* parse-sml/src/base/lib/TreeSeq.sml : 1.1-114.1 *)
(* molasses-file31.sml *)
...
structure TreeSeq =
  struct
    ...
    datatype 'a t =
    Leaf | Elem of 'a | Flat of 'a Seq.t | Node of int * 'a t * 'a t
    ...
    fun toArraySeq t =
      let
        ...
        fun put offset t =
          ...
          | Flat s =>
              Seq.applyIdx s (fn (i, x) => Array.update (a, offset + i, x))
          | Node (n, l, r) =>
              let
