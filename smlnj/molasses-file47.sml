(* parse-sml/src/lex/Lexer.sml : 1.1-672.1 *)
(* molasses-file47.sml *)
    ...
    fun next (src : Source.t) : Token.Pretoken.t option =
      let
        ...
        and advance_inStringThreeDigitEscapeSequence s args =
          if
            check LexUtils.isDecDigit at s andalso check LexUtils.isDecDigit at s + 1 andalso check
                                                                                                LexUtils.isDecDigit at s + 2
          then
            SOME (EndOfChar (s + 3))
          else
            error
        ...
        and advance_inStringFourDigitEscapeSequence s args =
          if
            check LexUtils.isHexDigit at s andalso check LexUtils.isHexDigit at s + 1 andalso check
                                                                                                LexUtils.isHexDigit at s + 2 andalso check
                                                                                                                                       LexUtils.isHexDigit at s + 3
          then
            SOME (EndOfChar (s + 4))
          else
            error
        ...
        (** Inside a string, expecting to be inside a \f...f\
          ...
          *)
        and advance_inStringFormatEscapeSequence s (args as { stringStart
                                                            , escapeStart
                                                            }) =
          if is backslash at s then
        ...
        fun loop_topLevel s =
          if isEndOfFileAt s then
            ...
            | #"\"" =>
                let val s' = advance_toEndOfString (s + 1) {stringStart = s} in
            ...
            SOME s' =>
              if is #"\"" at s' then
                ...
                error
                  { pos = slice (s', s' + 1)
                  , what =
                      "Character constant contains more than one character."
                  , explain = NONE
                  }
        ...
        and loop_afterDot s =
          if (is #"." at s) andalso (is #"." at s + 1) then
            ...
            let val huh = if (is #"." at s) then s + 1 else s in
              error
                { pos = slice (huh, huh + 1)
                , what = "Unexpected character."
        ...
        and loop_alphanumId s (args as {idStart, startsPrime, longStart}) =
          if check LexUtils.isAlphaNumPrimeOrUnderscore at s then
            ...
            in
              if
                Token.isReserved (Token.fromPre tok) andalso (isQualified orelse is
                                                                                   #"." at s)
              then
                error
                  { pos = srcHere
