(* parse-sml/src/parse-mlb/ParseAllSMLFromMLB.sml : 1.1-292.1 *)
(* molasses-file68.sml *)
...
structure ParseAllSMLFromMLB :
  ...
  struct
    ...
    fun popScope ({fixities} : basis) : {old : basis, popped : basis} =
      let val {old, popped} = InfixDict.popScope fixities in
    ...
    fun parse {skipBasis, pathmap} mlbPath : (FilePath.t * Ast.ast) Seq.t =
      let
        ...
        fun doSML (ctx : context) (basis, path, errFun) =
          let val {result = path, used} = expandAndJoin (# dir ctx) path in
              ...
              let
                val (mlbCache, basis', asts) =
                  if FilePathDict.contains mlbCache path then
                    ...
                    let
                      val _ =
                        printErr ("loading  " ^ FilePath.toUnixPath path ^ "\n")
                      val mlbSrc =
                        Source.loadFromFile path
                          handle OS.SysErr (msg, _) => errFun msg
        ...
        and doBasdec (ctx : context) ( mlbCache : mlb_cache
                                     , basis : basis
                                     , basdec : MLBAst.basdec
          ...
          | DecBasis {elems, ...} =>
              let
                fun doElem ((mlbCache, basis, asts), {basexp, basid, eq}) =
                  let
                    val (mlbCache, basis', asts') =
