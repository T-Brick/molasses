(* src/gen/Generated.sml : 1.1-72.1 *)
(* molasses-file95.sml *)
structure Generated :>
  sig
  ...
  struct
    ...
    fun singleton (fopt, g) =
      ...
      | SOME f =>
          ( StringDict.singleton (FilePath.toUnixPath f, GenFile.name g)
          , StringDict.singleton ((FileName.toString o GenFile.name) g, g)
          )
    fun insert (d, l) (fopt, g) =
      case fopt of
        NONE =>
          (d, StringDict.insert l ((FileName.toString o GenFile.name) g, g))
      | SOME f =>
          ( StringDict.insert d (FilePath.toUnixPath f, GenFile.name g)
          , StringDict.insert l ((FileName.toString o GenFile.name) g, g)
          )

    fun find (_, l) = StringDict.find l o FileName.toString
    fun findPath (d, l) =
      Option.mapPartial (find (d, l))
      o StringDict.find d
      o FilePath.toUnixPath
    val findName = find
    ...
    fun removeMark (d, l) = (d, StringDict.map GenFile.removeMark l)
