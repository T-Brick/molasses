(* src/file/WrappedFile.sml : 1.1-187.1 *)
(* molasses-file91.sml *)
structure WrappedFile :
  sig
  ...
  struct
    ...
    in
      ...
      fun makeLib path future =
        ...
        in
          Option.map
            (fn filename =>
               makeEmpty filename [StrExport.Lib (FileName.toString filename)]
                 future)
            filename_opt
        end
      ...
      let
        ...
        val (future, istr, istr_export) =
          ...
          | _ =>
              ...
              in
                ( Import.Open istr_t :: future
                , SOME (istr_t, exp_exports)
                , istr_e
                )
              end
      in
    ...
    fun name wfile =
      ...
      in
        if # get Control.use_source () andalso not (FileName.isLib name) then
      ...
      fun astToString ast =
        TerminalColorString.toString {colors = false}
          (PrettyPrintAst.pretty
             {ribbonFrac = 1.0, maxWidth = 80, tabWidth = 4, indent = 2}
             ast)
