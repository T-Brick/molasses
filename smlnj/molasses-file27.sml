(* parse-sml/src/base/lib/PPM.sml : 1.1-286.1 *)
(* molasses-file27.sml *)
...
structure PPM :
  ...
  struct
    ...
    fun niceify str =
      if String.size str <= 10 then
        str
      else
        String.substring (str, 0, 7) ^ "..."
    ...
    fun parse3 contents =
      let
        ...
        fun tok i =
          let val (lo, hi) = tokRange i in Seq.subseq contents (lo, hi - lo) end
        fun strTok i = Parse.parseString (tok i)
        ...
        fun intTok thingName i =
          let
            fun err () =
              raise
                Fail
                  ("error parsing .ppm file: cannot parse "
                   ^ thingName
                   ^ " from '"
                   ^ niceify (strTok i)
                   ^ "'")
        ...
        val _ =
          if numToks = numChannels + 4 then
            ...
            raise Fail ("error parsing .ppm file: too few color channels")
          ...
          let
            ...
            val _ =
              if c <= resolution then
                ...
                raise
                  Fail
                    ("error parsing .ppm file: channel value "
                     ^ Int.toString c
    ...
    fun parse6 contents =
      let
        ...
        val _ =
          if
            Seq.length contents > 2 andalso Char.isSpace (Seq.nth contents 2)
          then
            ()
          else
        ...
        fun chan i = Word8.fromInt (Char.ord (Seq.nth contents (cursor + i)))
    ...
    fun read filepath =
      let val contents = ReadFile.contentsSeq filepath in
        case Parse.parseString (Seq.subseq contents (0, 2)) of
          "P3" => parse3 contents
        | "P6" => parse6 contents
        | _ =>
            raise Fail "error parsing .ppm file: unknown or unsupported format"
      end
    ...
    fun write filepath image =
      let
        ...
        fun dumpPx i j =
          let val {red, green, blue} = elem image (i, j) in
            (dumpChan red; dumpChan green; dumpChan blue)
          end
