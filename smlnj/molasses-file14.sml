(* parse-sml/src/base/lib/Merge.sml : 1.1-109.1 *)
(* molasses-file14.sml *)
...
structure Merge :
  sig
    ...
    val writeMergeSerial : ('a * 'a -> order)
                           (* compare *)
                              -> 'a seq * 'a seq
                           (* (sorted) sequences to merge *)
                              -> 'a seq
                           (* output *)
                              -> unit

    val writeMerge : ('a * 'a -> order)
                     (* compare *)
                        -> 'a seq * 'a seq
                     (* (sorted) sequences to merge *)
                        -> 'a seq
  ...
  struct
    ...
    fun mergeSerial cmp (s1, s2) =
      let val out = AS.full (allocate (AS.length s1 + AS.length s2)) in
        writeMergeSerial cmp (s1, s2) out; out
      end
