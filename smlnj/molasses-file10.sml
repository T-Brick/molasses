(* parse-sml/src/base/lib/ArraySequence.sml : 1.1-246.1 *)
(* molasses-file10.sml *)
...
structure ArraySequence =
  struct
    ...
    val % = fromList
    fun toList s =
      SeqBasis.foldl (fn (list, x) => x :: list) [] (0, length s)
        (fn i => nth s (length s - i - 1))
    ...
    fun zipWith3 f (s1, s2, s3) =
      tabulate (fn i => f (nth s1 i, nth s2 i, nth s3 i))
    ...
    fun reduce f b s = SeqBasis.reduce GRAN f b (0, length s) (nth s)
     

    fun scan f b s =
      let val p = AS.full (SeqBasis.scan GRAN f b (0, length s) (nth s)) in
    ...
    fun filterIdx p s =
      AS.full
        (SeqBasis.filter GRAN (0, length s) (nth s) (fn i => p (i, nth s i)))
     

    fun equal eq (s, t) =
      length s = length t andalso SeqBasis.reduce GRAN
                                    (fn (a, b) => a andalso b)
                                    true
                                    (0, length s)
    ...
    fun inject (s, updates) =
      let val result = map (fn x => x) s in
        parfor GRAN (0, length updates)
          (fn i =>
             let val (idx, r) = nth updates i in AS.update (result, idx, r) end);

        result
      end
     

    fun applyIdx s f = parfor GRAN (0, length s) (fn i => f (i, nth s i))
               ...
               fun loop i j k =
                 if k >= hi then
                   ...
                   let
                     ...
                     val numHere =
                       Int.min (numAvailableHere, numRemainingInBlock)
                   in
                     for (0, numHere)
                       (fn z => A.update (result, k + z, nth inner (j + z)));
