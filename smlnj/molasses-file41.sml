(* parse-sml/src/base/TerminalColorString.sml : 1.1-405.1 *)
(* molasses-file41.sml *)
...
structure TerminalColorString :>
  ...
  struct
            ...
            let
              ...
              fun strip count i =
                if count >= n orelse i >= String.size s then
                  ...
                  let val tabstop = count + tabWidth - count mod tabWidth in
                    if tabstop <= n then
                      strip tabstop (i + 1)
                    else
                      let
    ...
    fun foreground color t =
      let val (a, t) = splitAttributes t in
        Attributes {size = size t, attr = setForeground a color, child = t}
      end

    fun background color t =
      let val (a, t) = splitAttributes t in
        Attributes {size = size t, attr = setBackground a color, child = t}
      end

    fun bold t =
      let val (a, t) = splitAttributes t in
        Attributes {size = size t, attr = setBold a, child = t}
      end

    fun italic t =
      let val (a, t) = splitAttributes t in
        Attributes {size = size t, attr = setItalic a, child = t}
      end

    fun underline t =
      let val (a, t) = splitAttributes t in
        Attributes {size = size t, attr = setUnderline a, child = t}
      end
    ...
    fun translate f t =
      ...
      | Append {left, right, ...} =>
          append (translate f left, translate f right)
      | Attributes {attr, child, ...} =>
          let val c = translate f child in
            Attributes {attr = attr, child = c, size = size c}
          end
    ...
    fun print_ out t =
      let
        val (printer, filedesc) =
          ...
          | _ =>
              (fn x => TextIO.output (TextIO.stdErr, x), Posix.FileSys.stderr)
