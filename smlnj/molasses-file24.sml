(* parse-sml/src/base/lib/Parse.sml : 1.1-185.1 *)
(* molasses-file24.sml *)
...
structure Parse =
  struct
    ...
    fun parseDigit char =
      ...
      in
        if code < code0 orelse code9 < code then NONE else SOME (code - code0)
      end
          ...
          if i >= n then
            ...
            | cc =>
                case parseDigit cc of
                  NONE => NONE
                | SOME dig =>
                    buildAfterPoint (m * 0.1) (x + m * (Real.fromInt dig))
                      (i + 1)
          ...
          if i >= n then
            ...
            | cc =>
                case parseDigit cc of
                  NONE => NONE
                | SOME dig =>
                    buildBeforePoint (x * 10.0 + Real.fromInt dig) (i + 1)
      in
        if n = 0 then
          NONE
        else if (c 0 = #"-" orelse c 0 = #"~") then
    ...
    fun parseString s = CharVector.tabulate (Seq.length s, Seq.nth s)
