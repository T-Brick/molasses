(* parse-sml/src/base/lib/Util.sml : 1.1-354.1 *)
(* molasses-file7.sml *)
...
structure Util :
  ...
  struct
    ...
    fun reportTime f =
      let val (result, tm) = getTime f in
        print ("time " ^ Time.fmt 4 tm ^ "s\n"); result
      end

    fun closeEnough (x, y) = Real.abs (x - y) <= 0.000001
    ...
    fun loop (lo, hi) b f =
      if lo >= hi then
        ...
        loop (lo + 1, hi) (f (b, lo)) f

    fun forBackwards (i, j) f =
      if i >= j then
        ()
      else
        (f (j - 1); forBackwards (i, j - 1) f)
    ...
    fun all (lo, hi) f =
      let fun allFrom i = (i >= hi) orelse (f i andalso allFrom (i + 1)) in
        allFrom lo
      end

    fun exists (lo, hi) f =
      let fun existsFrom i = i < hi andalso (f i orelse existsFrom (i + 1)) in
        existsFrom lo
      end
    ...
    fun copyListIntoArray xs arr i =
      case xs of
        [] => i
      | x :: xs => (Array.update (arr, i, x); copyListIntoArray xs arr (i + 1))
    ...
    fun summarizeArraySlice count toString xs =
      let
        ...
        val strs =
          if count <= 0 then
            raise Fail "summarizeArray needs count > 0"
    ...
    fun startTiming () = if not reportTicks then Time.zeroTime else Time.now ()

    fun tick t msg =
      if not reportTicks then
        t
      else
        let
          val t' = Time.now ()
