structure LibraryMap :> sig
  ...
  fun convert (conversions, _) s = Map.lookup conversions s
  ...
  in
    fun addFromString libmap str = str
      |> String.tokens isNewLine
      |> List.map (String.tokens isTab)
      |> (fn l => if List.length l = 0
                  then raise LibraryMapParseError
                  else (List.map tuplify (List.tl l), List.hd l))
