...
struct
  ...
  and create (dir, pathmap) cfile acc basdec =
    ...
    | DecPathMLB {path, token} => loadFile (dir, pathmap) token acc path makeMLB
    | DecPathSML {path, token} => loadFile (dir, pathmap) token acc path makeSML
    | DecBasis _ => raise Unsupported "MLB basis dec not supported"
    | DecLocalInEnd {locall, basdec1, inn, basdec2, endd} =>
        makeLocal (dir, pathmap) cfile create acc
